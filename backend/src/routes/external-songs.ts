...
router.post('/cookie', async (req, res) => {
  try {
    ...
    res.status(500).json({ message: '设置 Cookie 失败', error: (error as Error).message });
  }
});
...
router.get('/cookie/status', async (req, res) => {
  try {
    ...
    res.status(500).json({ message: '获取 Cookie 状态失败', error: (error as Error).message });
  }
});
 ...
 * @returns 响应数据
 */
async function forwardToQQMusicApi(path: string, query?: Record<string, string>): Promise<any> {
...
router.get('/search', async (req, res) => {
  try {
    ...
    const pageNo = Math.floor(parseInt(offset as string) / 20) + 1;
    ...
    res.status(500).json({ message: '搜索歌曲失败', error: (error as Error).message });
  }
});
...
router.get('/detail', async (req, res) => {
  try {
    ...
    res.status(500).json({ message: '获取歌曲详情失败', error: (error as Error).message });
  }
});
...
router.post('/batch-detail', async (req, res) => {
  try {
    ...
    res.status(500).json({ message: '批量获取歌曲详情失败', error: (error as Error).message });
  }
});
...
router.get('/url', async (req, res) => {
  try {
    ...
    res.status(500).json({ message: '获取歌曲地址失败', error: (error as Error).message });
  }
});
...
router.get('/lyric', async (req, res) => {
  try {
    ...
    res.status(500).json({ message: '获取歌词失败', error: (error as Error).message });
  }
});
...
router.get('/artist/:vendor/:id', async (req, res) => {
  try {
    ...
    const result = await forwardToQQMusicApi('/singer/songs', {
      singermid: id,
      page: (Math.floor(parseInt(offset as string) / parseInt(limit as string)) + 1).toString(),
      num: limit as string,
    });
    ...
    res.status(500).json({ message: '获取歌手单曲失败', error: (error as Error).message });
  }
});
 ...
 * 格式化 QQ 音乐搜索结果
 */
function formatQQSearchResult(data: any) {
  if (!data) return null;
 ...
 * 格式化 QQ 音乐歌手歌曲列表
 */
function formatQQArtistSongs(data: any) {
  if (!data) return null;
 ...
 * 格式化 QQ 音乐歌曲条目
 */
function formatQQSongItem(song: any) {
  if (!song) return null;
  ...
  const artists = song.singer?.map((s: any) => ({
    id: s.mid,
    name: s.name,
 ...
 * 格式化搜索结果（兼容旧格式）
 */
function formatSearchResult(data: any) {
  if (!data) return null;
 ...
 * 格式化歌曲条目（兼容旧格式）
 */
function formatSongItem(song: any) {
  if (!song) return null;
