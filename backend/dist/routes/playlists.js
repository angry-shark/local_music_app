...
// 添加歌曲到歌单
router.post('/:id/songs', auth_1.authenticateToken, async (req, res) => {
    try {
        const id = parseInt(req.params.id);
        ...
        // 获取当前最大 order
        const maxOrder = await prisma_1.default.playlistSong.aggregate({
            where: { playlistId: id },
            _max: { order: true }
        });
        const playlistSong = await prisma_1.default.playlistSong.create({
            data: {
                playlistId: id,
                songId,
...
// 从歌单移除歌曲
router.delete('/:id/songs/:songId', auth_1.authenticateToken, async (req, res) => {
    try {
        const id = parseInt(req.params.id);
        ...
        await prisma_1.default.playlistSong.deleteMany({
            where: {
                playlistId: id,
                songId: songId
            }
...
// 获取我的歌单
router.get('/user/my', auth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.user.userId;
        const playlists = await prisma_1.default.playlist.findMany({
            where: { userId },
